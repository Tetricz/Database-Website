...
SELECT
    ...
    END overtime_hours
FROM 
    (SELECT SUM(DATE_PART('day', scheduled_arrival_time - scheduled_departure_time) * 24 + 
                         DATE_PART('hour', scheduled_arrival_time - scheduled_departure_time)) AS hours_worked
    FROM flightassignment
    WHERE pilot = '730-51-8952'
    GROUP BY pilot) AS total_hours);

SELECT payrate, overtime_payrate 
FROM job 
WHERE job = 
    (SELECT job 
    FROM employee 
    WHERE social_security_num = '730-51-8952');

SELECT payrate * normal_hours AS salary, overtime_payrate * overtime_hours AS overtime_salary
FROM ((SELECT
        CASE    
            WHEN hours_worked - 40 <= 0 THEN hours_worked
            WHEN hours_worked - 40 > 0 THEN 40
        ...
        WHERE job = 
            (SELECT job 
            FROM employee 
            WHERE social_security_num = '730-51-8952') AS pay));
